...
def run_stage2_localization(
    ...
    # --- Prevent "jammer-at-origin" leakage ---
    # Use ENU reference from data centroid if configured.
    if getattr(config, "reference_mode", "jammer") == "data_centroid":
        _df_ref = pd.read_csv(input_csv)

        # IMPORTANT FIX: use config.env_column, not hard-coded "environment"
        ...
        if len(_df_ref) > 0:
            ...
            if verbose:
                print(f"✓ Stage-2 ENU reference set to data centroid: lat0={config.lat0:.6f}, lon0={config.lon0:.6f}")
    ...
    if config.filter_by_environment and config.environment != 'mixed':
        ...
        if len(load_result) == 4:
            ...
            df = load_result[0]
            # Fallback to config values if available, otherwise use data centroid
            lat0_rad = load_result[1] if len(load_result) > 1 else (
                np.radians(config.lat0) if config.lat0 is not None else np.radians(df['lat'].median()))
            lon0_rad = load_result[2] if len(load_result) > 2 else (
                np.radians(config.lon0) if config.lon0 is not None else np.radians(df['lon'].median()))
        os.unlink(filtered_csv)
    else:
        # FIXED: Unpack all returned values from load_data
    ...
    # Initialize theta to data centroid (NOT origin)
    all_positions = np.array([train_dataset[i][0][:2].numpy() for i in range(len(train_dataset))])
    data_centroid = all_positions.mean(axis=0)
    theta_init = data_centroid.astype(np.float32)

    # True jammer position in ENU
    # FIXED: data_loader.py now uses NEUTRAL frame (receiver centroid as origin)
    # and returns theta_true as the jammer position in that frame
    
    # Get lat0_rad, lon0_rad from data_loader output (already computed)
    # This ensures we use the same reference frame as the data
    if 'lat0_rad' not in dir() or lat0_rad is None:
        # Fallback: compute from data centroid
        lat0_rad = np.radians(df['lat'].median() if 'lat' in df.columns else config.jammer_lat)
        lon0_rad = np.radians(df['lon'].median() if 'lon' in df.columns else config.jammer_lon)
    
    # Get true_theta from data_loader if available, otherwise compute it
    ...
    if jammer_lat is not None and jammer_lon is not None:
        # Compute true_theta in the SAME frame as the data
        ...
        true_theta = None
        print("WARNING: jammer_lat/jammer_lon not set in config. Localization error will be N/A.")
    ...
    if verbose and true_theta is not None:
        ...
        print(f"True jammer position (ENU): [{true_theta[0]:.2f}, {true_theta[1]:.2f}] m")

    results = {'centralized': None, 'federated': {}, 'df': df}

    # Centralized training - FIXED: pass theta_true for localization error
    model, history = train_centralized(
        train_loader, val_loader, test_loader,
        theta_true=true_theta,  # ADDED: pass true jammer position
    ...
    results['centralized'] = {
        'theta_hat': theta_hat,
        'true_theta': true_theta,  # Store for reference
        'loc_err': loc_err,
        'test_mse': test_mse,
        'lat': lat_hat,
        'lon': lon_hat,
    ...
    if verbose:
        ...
        print(f"{'='*60}")
        loc_str = f"{loc_err:.2f} m" if true_theta is not None else "N/A (no jammer location)"
    ...
    if run_fl and config.run_federated:
        ...
        if config.partition_strategy == "device":
            # Robust: our datasets expose per-sample device indices as `device_idx`.
            # train_dataset is usually a torch.utils.data.Subset, so we extract labels from it.
            device_labels = get_device_labels_from_subset(train_dataset)

            if device_labels is not None and len(device_labels) > 0:
                if verbose:
                    n_devices = int(len(np.unique(device_labels)))
                    print(f"  Using device-based partitioning ({n_devices} devices)")
            else:
                print("  WARNING: Could not extract device labels from dataset. Falling back to geographic partitioning.")
    ...
    if generate_plots and HAS_STAGE2_PLOTS:
        ...
        stage2_plots = generate_stage2_plots(
            ...
            output_dir=plot_dir,
            env=env_name,
            true_jammer=(tuple(true_theta) if true_theta is not None else None),  # True jammer in ENU frame
            verbose=verbose
        )
...
def augment_stage2_dataset(
    ...
    dlat = np.radians(df_jammed['lat'].values - config.lat0)
    dlon = np.radians(df_jammed['lon'].values - config.lon0)
    x_real = R * dlon * np.cos(lat0_rad)
    y_real = R * dlat
    df_jammed['distance_to_jammer'] = np.sqrt(x_real**2 + y_real**2)
